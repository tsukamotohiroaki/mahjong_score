...
- Issueの質が実装の質を決める。雑なIssueは事故と手戻りを生むため、起票には時間をかけて丁寧に進める
- Issue駆動開発: 実装前にIssueを作成し、Issueを起点にブランチを切る
- 起票を依頼されてもすぐ作らない。「目的」「やること / やらないこと」を確認し、合意できたものだけ起票する
- 起票基準は着手時期ではなく「やる価値があるか」。価値を説明できないものは起票しない（AIは遠慮なく指摘する。最終判断はユーザーが行う）
- 起票時は必ず優先度ラベル（`priority: high` / `priority: medium` / `priority: low`）を付ける。AIが根拠付きで提案し、決定はユーザーが行う
- 1 Issue = 1目的（1 PRで閉じる粒度）。目的が複数になったらIssueを分割する
- 実装中にスコープが増えたら、その場で広げずサブIssueに切り出す
...
- Claude in Chrome: 応答速度の計測（操作してから画面が表示されるまでの時間）。守備範囲と実行手順は `docs/test-strategy.md` が正
...
- README・ドキュメントは文字数を減らす方向で修正する。「あると親切」は追加しない
