...
本ファイルは共通ルール（守備範囲・確認項目 ID・モード・報告フォーマット・制約）だけを持つ。実行の詳細は章別ファイルに分かれており、**実行する章のファイルだけを読む**。章番号（§0〜§16）は分割前から通しで維持している。

| 章 | ファイル | いつ読むか |
|---|---|---|
| §0 使う道具と、使えなかったときの扱い | [`references/tools.md`](references/tools.md) | **実行開始前に必ず** |
| §4〜5 事前準備・テストデータ ／ §15 後始末 | [`references/setup.md`](references/setup.md) | 実行開始時 |
| §6 シナリオ A ／ §11 DB 突合 | [`references/scenario-a.md`](references/scenario-a.md) | A 群の実行時 |
| §7〜10 シナリオ B・C・D・E | [`references/scenario-bcde.md`](references/scenario-bcde.md) | B〜E 群の実行時 |
| §12 既知の不具合・既知ノイズ | [`references/known-issues.md`](references/known-issues.md) | **判定・報告時に必ず** |
