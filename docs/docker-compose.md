...
## 構成の変遷：イメージの取得先をミラー経由に変えた

現在の `image:` の指定は、過去に2つの障害を踏まえて今の形になっている。**なぜこう書かれているか**を残しておく。

### 旧構成（〜2026-08-02）

```yaml
db:
  image: postgres:16    # 倉庫の指定なし → Docker Hub から取得
web:
  image: ruby:3.3       # 倉庫の指定なし・バージョンも浮動
```

この書き方には2つの弱点があった。

**弱点①：バージョンが固定されていない。**
`ruby:3.3` は「3.3 系の**最新**パッチ版」を指す**浮動タグ**で、中身が予告なく差し替わる。Gemfile は `ruby "3.3.10"` とパッチバージョンまで指定しているため、イメージ側が 3.3.11 に変わると bundler が不一致で停止する。

```
Your Ruby version is 3.3.11, but your Gemfile specified 3.3.10
```

**2026-07-12 にローカル環境で実際に発生した。** コードを一切変更していないのに突然壊れるため、原因に気づきにくい。当時は手元でイメージにタグを貼り直して凌いだが、根本対応ではなかった。

**弱点②：Docker Hub に到達できない環境がある。**
倉庫を省略すると Docker Hub になるが、ネットワークポリシーによっては到達できない。

**2026-07-29 以降、クラウド開発環境（Claude Code on the web / モバイル）で発生した。** Docker Hub のイメージ配信元 `production.cloudfront.docker.com` が組織のポリシーで遮断されており、`docker pull` が 403 で失敗する。

```
failed to copy: ... production.cloudfront.docker.com/... : Forbidden
```

イメージを取得できないため `docker compose up` が①の段階で止まり、**開発環境が一切立ち上がらない**。テスト実行も、push 前フック（テストを要求する）も通らず、作業が完全に止まった。

### 新構成

```yaml
db:
  image: mirror.gcr.io/library/postgres:16.13
web:
  image: mirror.gcr.io/library/ruby:3.3.10
```

**倉庫を Google のミラーに変え、バージョンをパッチまで固定した。** これで弱点①②が同時に解消する。
...
**旧構成の問題は「クラウド環境だけ取得できない」という非対称性にあった。** 手元の Mac では動くため気づきにくく、環境を移した瞬間に初めて壊れる。

新構成では**どちらの環境からも同じ窓口を使う**ため、この非対称性が消える。

### なぜミラーで解決するのか

`mirror.gcr.io` は Google が運用する **Docker Hub の公開ミラー**。Docker Hub のイメージをそのままコピーして配っている。

**イメージの中身は Docker Hub のものと同一。** Google が独自に何かを作っているわけではないため、取得したイメージの `ruby -v` は公式と一致する。変わるのは「どの窓口から受け取るか」だけ。

```
   イメージ名の指定と、実際にアクセスする先

   【旧】 image: ruby:3.3
                 └─ 倉庫の指定なし
                    ↓ 省略時は Docker Hub と解釈される
                    docker.io/library/ruby:3.3
                    → production.cloudfront.docker.com から取得 ← ここが遮断されている

   【新】 image: mirror.gcr.io/library/ruby:3.3.10
                 └────┬─────┘ └────┬───┘ └──┬──┘
                    倉庫を明示     何を     どの版
                    ↓
                    mirror.gcr.io から取得 ← 遮断されていない
```

Docker Hub のレート制限（無認証は 6 時間あたり 100 pull）を回避する目的でも広く使われている手法。

### トレードオフ

| | 内容 |
|---|---|
| **得たもの** | クラウド環境で `docker compose up` が通る。バージョン不一致による突然の起動不能がなくなる |
| **失ったもの** | 取得先が Google のミラーに依存する。ミラーが停止すると**新規取得**ができない |
| **影響範囲** | 影響を受けるのは「イメージを新しく取得するとき」だけ。すでにイメージを持っている環境（稼働中の本番 EC2 など）は起動・停止とも通常どおり動く |
| **戻し方** | `image:` の2行を元に戻すだけ。**取り返しのつかない変更ではない** |

### バージョンを上げるときの注意

Ruby のバージョン指定は**4箇所**にあり、すべて一致していないと bundler が停止する。上げるときは同時に更新する。
...
| `docker-compose.yml` | `mirror.gcr.io/library/ruby:3.3.10` |
