...
※ テストケース数は 2026年8月時点の実測値（再測定: `grep -rcE '^\s*it\b' spec/` / `grep -c "test(" e2e/*.spec.ts`）
...
```mermaid
flowchart TB
    e2e["E2E（Playwright）: 9ケース<br>JS挙動・ユーザー操作フロー<br>= お客さんの席を守る門番"]
    req["リクエストスペック（RSpec）: 61ケース<br>HTTP入出力・バリデーション<br>= 注文の受け渡しを守る門番"]
    model["モデルスペック（RSpec）: 36ケース<br>計算ロジック・不変条件<br>= 厨房の裏側を守る門番"]
    e2e --> req --> model
```
