...
「起きたらアプリの価値がなくなる」リスクと、その守り方。

| リスク | なぜ高いか | 守り方 |
|---|---|---|
| ① 順位点を黙って間違える<br>`Game#calculate_ranking_scores` | 計算バグは例外を出さず**間違った数字を出す**。クラッシュしないため気づけず、テストでしか守れない | `game_spec`（同点分配・境界値を網羅） |
| ② 不正データが混入し以後の計算を汚染する<br>ゼロサム検証・プレイヤー4人固定 | 入った瞬間に**全計算が汚染される**。入口で止めるのが最も安い | `game_spec`（3/5/0人・配列以外）＋トランザクション |
| ③ 入力導線から不正値が入る<br>±10万点・合計10万点ちょうど | ユーザーが毎回通る導線で、**不正値の唯一の入口** | リクエストスペック＋ E2E ＋ Vitest |

リスク③のクライアント側検証は UX のため。**正はサーバー側の検証**（`RoundScoreForm` に一本化。MPA・LIFF どちらの経路も同じ検証を通る）で、すり抜けた値も必ず拒否する。
...
|---|---|---|---|
| **RSpec モデルスペック** | 計算・不変条件（リスク①②） | ○ | ○（計算はサーバー側に一本化されているため両版で共有） |
...
リスクの高い箇所には厚く、単純な箇所は浅く。偏りは意図的なもの。具体的なテストケースは `spec/`・`e2e/`・`frontend/` のテスト名が唯一の情報源。
...
| 対象 | テスト | 厚み |
|---|---|---|
| リスク①② 計算・不変条件 | `spec/models/game_spec.rb` | ◎ 最厚（同点分配・境界値を個別に網羅） |
| リスク③ 点数入力（MPA / API） | `spec/requests/**/rounds_spec.rb` | ◎ 厚い（バリデーション・上書き・採番） |
| リスク③ の JS 挙動 | `e2e/score_input.spec.ts` | ○ E2E の大半をここに集中 |
...
| 対象 | テスト | 厚み |
|---|---|---|
| リスク③ の JS 挙動 | `frontend/app/games/[id]/rounds/new/page.test.tsx` | ◎ 厚い（合計・自動補完・送信可否・上書き・APIエラー） |
