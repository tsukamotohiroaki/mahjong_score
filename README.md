...
**Issue 駆動 + TDD（テスト駆動開発）** で、次の 5 ステップを 1 サイクルとして進めます。

| # | ステップ | やること | ねらい |
| --- | --- | --- | --- |
| 1 | Issue | イシューを起点にブランチを切る | 目的と完了条件を先に決める |
| 2 | 影響範囲の特定 | [`.claude/dependencies.md`](.claude/dependencies.md)（依存マップ）で、変更するコードを守っているテストを引く | 壊した箇所の取りこぼしを防ぐ |
| 3 | Red | 失敗するテストを先に書く | 仕様をテストで固定する |
| 4 | Green | テストが通る最小限の実装をする | 作りすぎを防ぐ |
| 5 | Refactor | テストを通したまま整理する | 動作を変えずに読みやすくする |

> ステップ 2 を挟むやり方を **TDAD（Test-Driven Agentic Development）** と呼んでいます。どのコードがどのテストに守られているかを事前に引けるので、AI に実装させても壊した箇所を取りこぼしません。
